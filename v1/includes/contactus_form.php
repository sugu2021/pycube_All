...
if (isset($_POST['contactus_sbt'])) {
	...
	$created = date("Y-m-d H:i:s");
        $query = "INSERT INTO `contactus`( `fname`, `lname`, `title`, `company`, `email`, `phone`, `message`, `datetime`) VALUES ('$fname','$lname','$title','$company','$email','$phone','$post_message','$created')";
        $add_whitepaper = mysqli_query($connection, $query);


    $file_names = array();
    $folder = '';
    $mailto = 'steve.l@pycube.com, sales@pycube.com'; //amanda.atrash@pycube.com
    $from_mail = $email;
    $from_name = $full_name;
    $replyto = $from_mail;
    $subject = "Contact Us from " . $full_name;
